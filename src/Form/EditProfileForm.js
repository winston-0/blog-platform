...
const EditProfileForm = () => {
  const dispatch = useDispatch();
    ...
    return (
      <React.Fragment>
        {contextHolder}
        <Form name="editProfile" form={form} onFinish={onFinish} style={{width: 320}} layout='vertical' colon={false} requiredMark={false}>
        <Form.Item initialValue={name} name='username' label='Username' rules={[
           {whitespace: true},
           {min: 3,
            max: 20}
        ]} >
        ...
        <Form.Item initialValue={email} name='email' label='Email address' rules={[
          {type: 'email',
        ...
        <Form.Item initialValue={profileImage} name='image' label='Avatar image (url)' rules={[
            {type: 'url',
        ...
        <Form.Item style={{marginBottom: 8}}>
          <Button type='primary' size='large' block htmlType='submit'>Save</Button>
        </Form.Item>
    </Form>
    </React.Fragment>
    )
